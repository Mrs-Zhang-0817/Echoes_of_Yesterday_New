...
export class Chapter04 {
  ...
  handleDown(point) {
    if (this.phase === 'signature') {
      ...
      // 电话热区：图片区域 (120, 260, 320×320)
      const px = 120, py = 260, pw = 320, ph = 320;
      const hit = point.x >= px && point.x <= px + pw &&
                  point.y >= py && point.y <= py + ph;
  ...
  render(ctx) {
    if (this.comic && !this.comic.isFinished) {
      this.comic.render(ctx, this.game.width, this.game.height);
      return;
    }
  ...
  drawPhone(ctx) {
    const img = this.game.images.ch4_phone;
    if (!img) { this._drawPhoneFallback(ctx); return; }

    // 电话图片在画布上的绘制区域（左偏，桌面上方）
    const px = 120, py = 260, pw = 320, ph = 320;
    ...
    if (this.phase === 'phone') {
      ...
      for (let i = 0; i < 3; i++) {
        const radius = Math.max(pw, ph) / 2 + 20 + i * 25;
    ...
    if (this.phase === 'ringing') {
      ...
      const scale = isPulsing ? 1 + 0.06 * Math.sin(pulseElapsed / 0.2 * Math.PI) : 1;
  ...
  // 图片加载失败时的回退（保留原始 Canvas 绘制）
  _drawPhoneFallback(ctx) {
    ...
    if (this.phase === 'phone') {
      const breath = 0.1 + 0.3 * (0.5 + 0.5 * Math.sin(this.totalTime * 2 * Math.PI / 1.5));
      const cx = baseX + baseW / 2, cy = baseY + baseH / 2;
      for (let i = 0; i < 3; i++) {
    ...
    ctx.fillStyle = '#5a3020'; ctx.fill();
    ctx.shadowBlur = 0; ctx.shadowOffsetY = 0;
    ctx.fillStyle = '#4a2518';
    roundedRect(ctx, baseX + 15, baseY + 12, baseW - 30, 8, 3); ctx.fill();
    for (let i = 0; i < 10; i++) {
