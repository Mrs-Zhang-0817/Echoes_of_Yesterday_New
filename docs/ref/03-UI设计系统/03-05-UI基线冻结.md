---
source: Echoes_of_Yesterday_UI_v1.1/docs/UI_BASELINE.md
date: 2026-07-30
status: archived-reference
related: 03-06-UI更新日志.md, 03-03-UI资产清单.md
---

# 03-05 UI 基线冻结

> 原始文档：《昨日重现》Production UI Baseline  
> 版本：UI v1.0  
...
pictures/
  ...
  按钮图.png                             主界面按钮资产
记忆恢复报告新底图/
  第一章.png … 第九章.png                章节整图（v1.0 不含第十章）
...
## 4. SHA-256 校验和（v1.0 冻结基准）

冻结时通过 SHA-256 记录核心资源。后续变更必须在 `UI_CHANGELOG.md` 中升级版本并说明原因。
...
剧情、关卡和存档系统只允许调用 `src/ui/UIManager.js`。新增章节优先只改 `src/ui/memory-report-config.json`。任何视觉修改必须先升级 UI 版本、记录变更原因，并重新执行九章、主界面和导航回归测试。
