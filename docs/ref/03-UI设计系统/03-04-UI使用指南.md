---
source: Echoes_of_Yesterday_UI_v1.1/docs/UI_ASSET_USAGE_GUIDE.md
date: 2026-07-30
status: archived-reference
related: 03-02-UI集成指南.md, 03-03-UI资产清单.md
---

# 03-04 UI 使用指南

> 原始文档：《昨日重现》UI 与图片资源使用指南  
> 版本：UI v1.1  
> 打包状态：预览待确认
...
- `http://localhost:8080/memory-report-artwork.html?chapter=chapter_01`（替换 `chapter_01` 至 `chapter_10`）
...
```js
import { openMainMenu, openMemoryReport } from "./src/ui/UIManager.js";
...
### B. 正式 Artwork Memory Report（`记忆恢复报告新底图/`）
- `第一章.png` 至 `第十章.png`：完整章节报告底图
- `memorybutton1-transparent.png`：返回主界面（左）
- `memorybutton2-transparent.png`：查看记忆档案（中）
- `memorybutton3-transparent.png`：继续昨日（右）
...
### D. 关卡美术素材
- `第一章/`、`第四章·警局/`、`第四章第一视角情景演示/`、`美术素材/`、`assets/images/`
...
5. `assets/fonts/`（当前仓库未包含字体文件，浏览器使用楷体/仿宋等本机字体回退。最终跨设备发布前需补齐已授权字体）

### 旧版兼容层
`src/ui/screens/`、`src/ui/components/`、`src/ui/materials/`、`src/ui/animations/` 与 `src/memory-report-phase4.js` 保留，不用于创建新的 Artwork 页面。
...
2. 清晰度数字与进度条推进
3. `memoryFrom == memoryTo` 的静止章节（Ch3）
4. 继续下一章
5. 返回主界面
6. Chapter 10 达到 100%
