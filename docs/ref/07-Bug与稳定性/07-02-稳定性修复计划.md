---
source: docs/stability-repair-plan.md
date: 2026-07-30
status: completed
...
1. 补 `.gitignore`：排除根目录调试截图 `ch*.png`、`.playwright-mcp/`、`.tmp_feishu_imgs/`、`build_out/screenshots/`
...
- 产出 `docs/bug-inventory.md`（见 07-01-Bug清单.md）
- 每条记录：章节 / 复现步骤 / 卡死表现 / 疑似原因 / 优先级
- **修复前先有完整清单，禁止边测边改**
...
每个修复单独 commit，保证可单步回退：

1. **fix(ch08): 签字关提交/清除按钮在 writing 阶段可点击** (8d57cba)
   - handleDown 先判按钮区命中，未命中才开始笔画
2. **fix(ch07): 补 markChapterComplete 使进度落库** (dcccf81)
   - P1-1 在 `_complete = true` 处补写存档
3. **fix: 刷新后从已完成章的下一章恢复，不再回退一章** (8b43752)
   - P1-2 恢复时取 min(N+1, 10)

### P1 待修复

- ch05 memory=40 显示 35%、ch06 memory=52 显示 45%（统一为实际值或纯叙事文案）
- P1-4 签名阈值调参（80 → 60/放宽笔画要求）

### P2 待修复

- DebugAPI inspect 输出过滤大数组
- 3 个缺 Echoes dist 的测试标记 skip
...
- ⏸ **ChapterManager 章节完成契约**：经评估已健壮——switchTo 有 registry.has 守卫、完成统一走 update() 检测 isComplete→overlay→next()→switchTo，无旁路风险，暂不改动
- ⏸ **每章生命周期检查**（定时器/监听/粒子清理）：属 10 章大范围审计，暂停避免引入回归

## Phase 4 · 美术替换协议（与修复并行）
